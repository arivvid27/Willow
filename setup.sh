...
if $NEED_KEYS; then
  ...
  prompt "Supabase service_role key (for backend only)"
  ...
  prompt "Backend URL for mobile (default uses LAN IP)" "http://$LAN_IP:8000"
  read -r MOBILE_API_URL
  MOBILE_API_URL="${MOBILE_API_URL:-http://$LAN_IP:8000}"

  echo ""

  # ── Write backend .env ────────────────────────────────────────────────────
  if $HAS_BACKEND && ! $BACKEND_ENV_EXISTS; then
    cat > "$BACKEND_ENV" << EOF
# Willow Backend — generated by setup.sh
SUPABASE_URL=$SUPABASE_URL
SUPABASE_SERVICE_ROLE_KEY=$SUPABASE_SERVICE_KEY
GEMINI_API_KEY=$GEMINI_KEY
FRONTEND_URL=http://localhost:3000
EOF
    success "Created $BACKEND_ENV"
  fi

  # ── Write frontend .env.local ─────────────────────────────────────────────
  if $HAS_FRONTEND && ! $FRONTEND_ENV_EXISTS; then
    cat > "$FRONTEND_ENV" << EOF
# Willow Frontend — generated by setup.sh
